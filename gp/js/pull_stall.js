...
(function () {
  ...
  let domServe = () => {
    ...
    if (!JSStore.avail().locale) document.querySelector(`#ModelZones`).className = `-Zz`;
      ...
      S.emit(`pullStall`, {
      	locale: JSStore.avail().locale, log_secs: JSStore.avail().log_secs, mug: JSStore.avail().mug, pullStall: JSStore.avail().pullStall});
    }
  ...
  S.on(`pullStall`, J => {
    ...
    if (JSStore.avail().log_secs === J.log_secs) {
      ...
      ModelSource.innerHTML = M.modelStringify(J.ModelPullStall);
